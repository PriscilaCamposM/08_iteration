...
```{r density plot} 
nematode %>%
  pivot_longer(cols = mel_s:re) %>% #43*16 = 688
  arrange(name) %>%
  ggplot(aes(x = value)) +
  geom_density() +
...
nematode %>%
  ...
  #mutate(bndvi = (nir - b)/(nir + b))%>%
  #mutate(gndvi = (nir - g)/(nir + g))
...
thats why we created the function, it is easier, instead of writing three times ndvi in the previous step.
...
nematode_meancv$variable[[i]] <- varname
nematode_meancv$mean[[1-2]] <- round(statsum$mean,3)
...
mutate(splota_better = pmap(list(.df = data,
                                 .r2 = r2,
                                 .rv = resp_var,
                                 .ev = exp_var
                                 ),
...
nematode_mods_sel_plots$splot[1]
```
